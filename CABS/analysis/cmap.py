...
class ContactMapFactory:
    def __init__(self, chains1, chains2, temp):
        ...
        self.res1 = sorted([(k, (v[0], v[-1])) for k, v in res1.items()], key=lambda x: x[1][0])
        self.res2 = sorted([(k, (v[0], v[-1])) for k, v in res2.items()], key=lambda x: x[1][0])
    ...
    def mk_cmap(self, traj, thr, frames=None, replicas=None):
        ...
        for rep in traj[replicas,]:
            ...
            for fra in rep[frames,]:
                ...
                cmtx += ncmtx
                nframes += 1
            resl.append(ContactMap(cmtx, [i[0] for i in self.res1], [i[0] for i in self.res2], nframes))
        return resl
...
class ContactMap:
    ...
    def save_fig(self, fname, fmt='svg', norm_n=False, break_long_x=50, colors_lst=['#ffffff', '#f2d600', '#4b8f24', '#666666', '#e80915', '#000000']):
        ...
        grid = plt.GridSpec(len(chunks) + 1, 1, height_ratios=([len(self.s2) for i in chunks] + [len(self.s2) * .25]))
        vmax = self.n if norm_n else np.max(self.cmtx)
        if vmax < 5:
            vmax = 1 if norm_n else 5

        colors = matplotlib.colors.LinearSegmentedColormap.from_list('bambi', ['#' + color if '#' not in color else color for color in colors_lst]
)
        ...
        for n, chunk in enumerate(chunks):
            ...
            settings = (
                        (list(np.array(self.s1)[chunk,]), len(chunk), sfig.set_xticks, sfig.set_xticklabels),
        ...
        fig.get_axes()[0].set_title('Contacts frequency' if self.n > 1 else "Contact map")
    ...
    def save_histo(self, fname, all_inds_stc2=True, fmt='svg'):
        ...
        peptH = mk_histo(plt.subplot(grid[0, 0]), pep_vls, pep_lbls, ylim=(0, max([.05] + pep_vls)))[0]
        ...
        targAH = mk_histo(plt.subplot(grid[-1, 0]), trg_vls_all, self.s1, ylim=(0, max_y))[0]
