...
def _load_json_config(config_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise FileNotFoundError(f"Configuration file '{config_file}' not found in CABS/data/")
    except json.JSONDecodeError as e:
        raise json.JSONDecodeError(f"Invalid JSON in configuration file '{config_file}': {e.msg}", e.doc, e.pos)
...
def get_config_section(config_name: str, section: str) -> Any:
    ...
    if section not in config:
        raise KeyError(f"Section '{section}' not found in configuration '{config_name}'")
