"""
Modern utility functions for CABS with comprehensive type annotations.
"""
...
from typing import Dict, List, Tuple, Union, Optional, Any, Callable, Iterator, TextIO
...
from CABS.constants import (
    ...
    CONFIG_HEADER, AminoAcidCode
)

__all__ = [
    'CABS_SS', 'CABS_SS_REVERSE', 'SIDECNT', 'AA_NAMES', 'AA_SUB_NAMES',
    'RANDOM_LIGAND_LIBRARY', 'SecondaryStructure', 'AminoAcid',
    'aa_to_short', 'aa_to_long', 'check_peptide_sequence', 'smart_flatten',
    'kabsch', 'convert_cg_to_all', 'SCModeler', 'InvalidAAName',
    'next_letter', 'rmsd', 'dynamic_kabsch', 'fix_residue', 'EprGenerator',
    'random_rotation_matrix', 'line_count', 'ranges'
]
...
def _load_random_ligand_library() -> npt.NDArray[np.float64]:
    ...
    try:
        ...
        with files('CABS').joinpath('data/data0.dat').open('rb') as f:
...
class SCModeler:
    ...
    def _calc_nodes_line(
        old_v: npt.NDArray[np.float64], 
        new_v: npt.NDArray[np.float64]
    ...
    def rebuild_one(self, vec: npt.NDArray[np.float64], sc: bool = True) -> npt.NDArray[np.float64]:
    ...
    def _calculate_traj(self, traj: TrajectoryArray, sc: bool = False) -> TrajectoryArray:
...
def smart_flatten(l: List[str]) -> List[int]:
    ...
    for i in l:
        if '-' in i:
            ...
            if beg > end:
                raise Exception(f'The left index({beg}) is greater than the right({end})')
...
class EprGenerator:
    ...
    def generate_epr(self, variant: Literal['easy', 'normalized', 'multiplied'] = 'easy', 
                     outfile: str = 'outfile.txt') -> None:
        """Generate EPR coefficients using specified variant."""
        self.outfile = outfile
        self.variant_dictionary[variant]()

    def save_weights_file(self, outfile: str = '', new_weights: Optional[List[float]] = None) -> None:
        """Save weights to file."""
        if new_weights is None:
            new_weights = []
        with open(self.outfile, 'w') as outfile_handle:
            for residue, new_weight, external_parameter in zip(self.residues, new_weights, self.external_parameters):
    ...
    def normalized(self) -> None:
        ...
        new_weights = len(self.external_weights) * np.array(self.external_weights) / sum(self.external_weights)
...
def convert_cg_to_all(
        ...
        for line in f:
            if line.startswith('ENDMDL'):
                ...
                if match:
                    ...
                    tmp.write(line)
    if renumber_flag:
        sync_residues(input_pdb_path=Path(reference_pdb), 
                      output_pdb_path=Path(tmp))
    ...
    try:
        ...
        logger.critical(module_name="CG2ALL", 
...
def sync_residues(input_pdb_path: Path, output_pdb_path: Path) -> str:
    ...
    if not HAS_PANDAS:
        raise ImportError("pandas and biopandas are required for sync_residues function")
