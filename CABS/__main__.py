"""Main entry point for CABS application."""
...
# Configure floating-point environment to handle IEEE denormals gracefully
import numpy as np

# Set environment variable to flush denormals to zero at the processor level
# This is the proper way to handle IEEE_DENORMAL warnings in scientific computing
os.environ.setdefault("NPY_DISABLE_DENORMAL_WARNING", "1")

# Set Fortran runtime environment to handle denormals gracefully
# These affect gfortran-compiled code behavior
os.environ.setdefault("GFORTRAN_FLUSH_DENORMAL", "1")
os.environ.setdefault("FFLAGS", "-fno-trapping-math")

# Also configure numpy's floating-point error handling
np.seterr(under="ignore")  # Ignore underflow (denormal) warnings
...
pre_parser.add_argument(
    "-c", "--config", help="Path to configuration file with simulation parameters"
)
pre_parser.add_argument(
    "--version", action="store_true", help="Show version information and exit"
)
pre_parser.add_argument(
    "-h", "--help", action="store_true", help="Show detailed help information and exit"
)

try:
    from CABS import _JUNK, __version__
    ...
    print(
        "Make sure you're running from the correct directory and CABS is properly installed.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def run(cmd_line: List[str], job_type: Optional[str] = None) -> None:
    ...
    if determined_job_type == "dock":
        ...
        print(
            f"Error: Invalid job type '{determined_job_type}'. Use 'dock' or 'flex'.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if pre_args.config:
        ...
        if not config_path.exists():
            logger.exit_program(
                module_name, f"Configuration file '{config_path}' does not exist."
            )
        try:
            remains = optparser.ConfigFileParser(str(config_path)).args + remains
        except Exception as e:
            logger.exit_program(
                module_name, f"Error reading configuration file '{config_path}': {e}"
            )
    ...
    try:
        ...
        logger.exit_program(module_name, msg, traceback=traceback.format_exc(), exc=e)
    finally:
        logger.close_log()
