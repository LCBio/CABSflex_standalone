"""Modern logging module for CABS with Python 3 improvements."""
...
def setup(log_level: int = 2, remote: bool = False, work_dir: Union[str, Path] = '', 
          save_dssp: bool = False, save_ss: bool = False, save_restraints: bool = False) -> None:
    """
...
def log(module_name: str = 'MISC', msg: str = 'Processing', l_level: int = 2, 
    ...
    if l_level <= _log_level:
        ...
        if len(msg) < _line_break:
            ...
            for line_number in range(1, len(lines) - 1):
                ...
                out.write(line)
            final_line = _last_line_format.format(' ', lines[-1], strftime('(%H:%M:%S)', t))
...
def to_file(filename: Union[str, Path] = '', content: str = '', msg: str = '', 
           allow_err: bool = True, traceback: bool = True) -> None:
    """
    ...
    if filename:
        ...
        try:
            ...
            if allow_err:
                ...
                exit_program(module_name=_name, msg=f'IOError while writing to: {file_path}', 
                           traceback=traceback)
    if msg:
        log_file(module_name=_name, msg=msg)


def exit_program(module_name: str = _name, msg: str = 'Shutting down', 
                traceback: Optional[str] = None, exc: Optional[Exception] = None) -> None:
    """
...
class ProgressBar:
    ...
    def __init__(self, total: int = 100, module_name: str = '', job_name: str = '', 
                 out: TextIO = stderr, delay: float = 0, start_msg: str = '') -> None:
        """
        ...
        if self.job_name:
            log(module_name=self.module_name, msg=f'{self.job_name} running...', out=self.stream)
    ...
    def write(self) -> None:
        ...
        self.stream.write(self.FORMAT.format(
            self.prefix, 
            coloring(msg=f'{self.module_name}:'), 
            bar, 
            percent
        ))
        self.stream.flush()
...
class CabsObserver(Thread):
    ...
    def __init__(self, interval: float = 0.5, progress_file: Optional[Union[str, Path]] = None, 
                 job_name: str = 'CABS simulation', msg: str = '') -> None:
        """
